...
export const printAddress = ({ name, address: {street, number, city, country} }) => {return `${name} lives at ${number} ${street}, ${city}, ${country}`}
...
export const totalSneakerCount = (brands) => {
    ...
    for (let brand in brands) {
        shoeTotal = shoeTotal + brands[brand].shoes.length;
    }
    return shoeTotal;
}
...
export const convertToArray = (someObject) => {
    let someArray = [];
    for (let key in someObject) {
        someArray.push([key, someObject[key]])
    }
    return someArray;
}
